		...
		<form id="em-transactions-table-form" class="transactions-filter" action="" method="post">
			...
			<div class="tablenav">
				<div class="alignleft actions">
					<select name="limit">
						<option value="<?php echo $this->limit ?>"><?php echo sprintf(esc_html__emp('%s Rows','dbem'),$this->limit); ?></option>
					...
					<?php if( is_object($context) && get_class($context)=="EM_Event" ): ?>
					<?php esc_html_e_emp('Displaying Event','dbem'); ?> : <?php echo $context->event_name; ?>
					<?php elseif( is_object($context) && get_class($context)=="EM_Person" ): ?>
					<?php esc_html_e_emp('Displaying User','dbem'); echo ' : '.$context->get_name(); ?>
					<?php endif; ?>
				</div>
				<?php 
			...
			foreach($transactions as $key => $transaction) {
				...
				<tr valign="middle" class="alternate">
					...
					<td class="column-trans-note-id">
						...
						<span class="trash"><a class="em-transaction-delete" href="<?php echo em_add_get_params($_SERVER['REQUEST_URI'], array('action'=>'transaction_delete', 'txn_id'=>$transaction->transaction_id, '_wpnonce'=>wp_create_nonce('transaction_delete_'.$transaction->transaction_id.'_'.get_current_user_id()))); ?>"><?php esc_html_e_emp('Delete','dbem'); ?></a></span>
						<?php endif; ?>
					</td>
			    </tr>
				<?php
		...
		if( is_object($context) && (get_class($context)=="EM_Booking" || get_class($context)=="EM_Multiple_Booking" ) && $context->can_manage('manage_bookings','manage_others_bookings') ){
			...
			if( get_option('dbem_multiple_bookings') && $context->can_manage('manage_others_bookings') ){
				...
				if( $EM_Multiple_Booking !== false ){
					$booking_condition = 'tx.booking_id = '.$EM_Multiple_Booking->booking_id;
				}
			}
