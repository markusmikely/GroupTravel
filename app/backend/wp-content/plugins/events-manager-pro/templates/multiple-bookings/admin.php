...
if( is_object($EM_Multiple_Booking) && !$EM_Multiple_Booking->can_manage() ){
	?>
	<div class="wrap"><h2><?php esc_html_e_emp('Unauthorized Access','dbem'); ?></h2><p><?php echo esc_html_e_emp('You do not have the rights to manage this event.','dbem'); ?></p></div>
	<?php
	return false;
}
?>
<div class='wrap' id="em-bookings-admin-booking">
	<div class="icon32" id="icon-bookings"><br></div>
  	<h2>
  		<?php esc_html_e_emp('Edit Booking', 'dbem'); ?>
  	</h2>
	...
	<div id="post-body">
		<div id="post-body-content">		
			<div class="stuffbox">
				...
				<div class="inside">
					...
					<div class="em-booking-person-details">
						...
						<?php if( $no_user ): ?>
						<input type="button" id="em-booking-person-modify" value="<?php esc_attr_e_emp('Edit Details','dbem'); ?>" />
						<?php endif; ?>
					</div>
					<?php if( $no_user ): ?>
					<form action="" method="post" class="em-booking-person-form">
						<div class="em-booking-person-editor" style="display:none;">
							...
							<input type="submit" class="em-booking-person-modify-submit" id="em-booking-person-modify-submit" value="<?php esc_attr_e_emp('Submit Changes', 'dbem'); ?>" />
							<input type="button" id="em-booking-person-modify-cancel" value="<?php esc_attr_e_emp('Cancel','dbem'); ?>" />
						</div>
					</form>	
				...
				<div class="inside">
					...
					<div>
						<form action="" method="post" class="em-booking-single-status-info">
							<strong><?php esc_html_e_emp('Status','dbem'); ?> : </strong>
							<?php echo $EM_Multiple_Booking->get_status(); ?>
							<input type="button" class="em-booking-submit-status-modify" id="em-booking-submit-status-modify" value="<?php esc_attr_e_emp('Change', 'dbem'); ?>" />
							<input type="submit" class="em-booking-resend-email" id="em-booking-resend-email" value="<?php esc_attr_e_emp('Resend Email', 'dbem'); ?>" />
						...
						<form action="" method="post" class="em-booking-single-status-edit">
							...
							<?php esc_html_e_emp('Send Email','dbem'); ?>
							<input type="submit" class="em-booking-submit-status" id="em-booking-submit-status" value="<?php esc_attr_e_emp('Submit Changes', 'dbem'); ?>" />
							<input type="button" class="em-booking-submit-status-cancel" id="em-booking-submit-status-cancel" value="<?php esc_attr_e_emp('Cancel', 'dbem'); ?>" />
							...
							<br /><em><?php echo wp_kses_data(__emp('<strong>Notes:</strong> Ticket availability not taken into account when approving new bookings (i.e. you can overbook).','dbem')); ?></em>
							</p>
						</form>
					</div>
					<form action="" method="post" class="em-booking-form">
						...
						<p class="em-booking-single-edit">
							<input type="submit" class="em-booking-submit" id="em-booking-submit" value="<?php esc_attr_e_emp('Submit Changes', 'dbem'); ?>" />
							<input type="button" class="em-booking-submit-cancel" id="em-booking-submit-cancel" value="<?php esc_attr_e_emp('Cancel', 'dbem'); ?>" />
			...
			<div id="em-booking-notes" class="stuffbox">
				...
				<div class="inside">
					<p><?php esc_html_e_emp('You can add private notes below for internal reference that only event managers will see.','dbem'); ?></p>
					<?php foreach( $EM_Multiple_Booking->get_notes() as $note ): 
						$user = new EM_Person($note['author']);
					?>
					<div>
						<?php echo sprintf(esc_html_x_emp('%1$s - %2$s wrote','[Date] - [Name] wrote','dbem'), date(get_option('date_format'), $note['timestamp']), $user->get_name()); ?>: 
